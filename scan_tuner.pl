...
# tuner address, either Tuner ID or an IP address
my $TUNER_ADDR = $TUNER_ID;

# which tuner to scan
my $TUNER = '/tuner0/';

# directory to write results into (optional, and in addition to sending to the URL)
my $DATA_DIR;

our ($opt_h,$opt_o,$opt_p,$opt_t,$opt_u,$opt_x,$opt_n,$opt_w,$opt_d);
getopts('ho:p:t:u:x:n:w:d');
help() if ($opt_h);
my $DEBUG = $opt_d;
...
unless ($TUNER eq '/tuner0/' || $TUNER eq '/tuner1/' || $TUNER eq '/tuner2/' || $TUNER eq '/tuner3/') {
  print "Invalid tuner ID.  Must be /tuner0/ or /tuner1/ or /tuner2/ or /tuner3/ \n";
  exit 1;
}
...
if ($opt_n) {
  if ($opt_n !~ /^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$/) {
    print "Invalid tuner IP.  Must be IPv4 numerical address.\n";
        exit 1;
  }
  $TUNER_ADDR = $opt_n;
}
...
my ($uid, $gid);
$DATA_DIR = $opt_w if $opt_w;
if ($DATA_DIR) {
  ($uid, $gid) = (stat($DATA_DIR))[4, 5]
}
...
SCAN: while(1) {
  ...
  # scan tuner
  print "trying to run $CONFIG_PROGRAM $TUNER_ADDR scan $TUNER\n" if $DEBUG;
  open SCAN, "$CONFIG_PROGRAM $TUNER_ADDR scan $TUNER |" or die "can't run scan";
  while(<SCAN>) {
    print $_ if $DEBUG;
  ...
  if (length($json) < 500) {
    ...
    if ($DATA_DIR) {
      print "Writing results to directory $DATA_DIR\n" if $DEBUG;

      open(FH, '>', "$DATA_DIR/.~last.json");
      print FH $json;
      close(FH);

      my $monthly = monthlyFileName();
      open(FH, '>>', "$DATA_DIR/$monthly");
      print FH $json;
      close(FH);

      chown $uid, $gid, "$DATA_DIR/.~last.json", "$DATA_DIR/$monthly";

      find(\&monthlyFileSpec,  $DATA_DIR);
    }
  }
}
...
sub monthlyFileName {
  ...
  $mon  += 1;
  $year += 1900;
  return sprintf ".~%04d-%02d.json", $year, $mon;
}

sub monthlyFileSpec {
  if (/^\.~[0-9]{4}-[0-9]{2}\.json/s && $_ ne monthlyFileName()) {
    my $newname = $_;
    $newname =~ s/\.~//;
    print "Compressing and renaming $_\n" if $DEBUG;
    if (system("gzip", $_) == 0) {
      rename "$_.gz",  "$newname.gz";
      chown $uid, $gid, "$newname.gz";
    }
  }
}
...
-n Tuner IP.  May be needed in addition to -x when network doesn't allow discovery (Docker).
-w Write results into a directory in addition to sending to the URL.
